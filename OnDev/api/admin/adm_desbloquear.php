...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id']) && isset($_POST['tipo'])) {
    ...
    // Preparar e executar a consulta para desbloquear o registro
    $sql = "UPDATE $table_name SET bloqueado = 0 WHERE id = ?";
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            // Redirecionar após o desbloqueio ser realizado com sucesso
            header("Location: dash_adm.php");
            exit();
        } else {
            echo "Erro ao executar a operação de desbloqueio: " . $stmt->error;
        }
